...
def savedat(np.ndarray[np.float64_t, ndim=2] arr, int nsteps, double Ts, double runtime,
            np.ndarray[np.float64_t, ndim=1] ratio, np.ndarray[np.float64_t, ndim=1] energy,
    ...
    Arguments:
	  ...
	  ratio (float[nsteps+1]) = array of acceptance ratios per MCS;
	  energy (float[nsteps+1]) = array of reduced energies per MCS;
	  order (float[nsteps+1]) = array of order parameters per MCS;
...
def get_order(double[:, ::1] arr, int nmax):
    ...
    Qab = Qab / (2.0 * int(nmax) * int(nmax))  # Explicitly cast operands to double

    # Calculate eigenvalues using eigvalsh
...
def MC_step(double[:, ::1] arr, double Ts, int nmax):
    ...
    cdef np.ndarray[np.float64_t, ndim=2] random_comparison = np.random.uniform(0.0, 1.0, size=(nmax, nmax))
    cdef np.ndarray[np.int_t, ndim=2] point_picked = np.zeros((nmax, nmax), dtype=np.int32)
